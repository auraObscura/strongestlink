...
function UserProfilePage(props) {
  ...
  const loadMyUserProfile = async () => {
    if (myUser) {
      const response = await StrongestLinkApi.getUserProfileByID(
        myUser.profile
      );
    ...
    if (userProfile) {
      ...
      if (userProfile.friends) {
        for (const friendProfileID of userProfile.friends) {
          newFriends.push(
            await StrongestLinkApi.getUserProfileByID(friendProfileID)
          );
        }
      }
      ...
      if (user.requests_received) {
        for (const requestID of user.requests_received) {
          newFriendRequests.push(
            await StrongestLinkApi.getFriendRequestByID(requestID)
          );
        }
      }
  ...
  const handleAddFriend = async () => {
    ...
    const response = await StrongestLinkApi.createFriendRequest(
      friendRequestData
    );
    if (response) {
  ...
  const handleRemoveFriend = async () => {
    ...
    const response = await StrongestLinkApi.deleteFriend(
      myUser.profile,
      userProfileData
    );
    loadUserProfile();
  ...
  const handleAcceptRequest = async (requestID) => {
    ...
    const response = await StrongestLinkApi.acceptFriendRequest(
      requestID,
      friendRequestData
    );
    loadUser();
  ...
  const renderAddFriendButton = () => {
    if (userProfile && myUser) {
      if (
        ...
        !userProfile.friends.includes(myUser.profile)
      ) {
        return (
  ...
  const handleEditProfile = async (event) => {
    ...
    if (uploadResponse) {
      ...
      const response = await StrongestLinkApi.editProfile(
        myUser.profile,
        userProfileData
      );
      loadUserProfile();
    ...
    <section>
      <div className="profile-container">
        ...
        {props.user.username == user.username && (
          ...
          >
            {wantToEdit ? "Cancel Edit" : "Edit Profile"}{" "}
          </button>
        )}

        {wantToEdit && props.user.username == user.username && (
          <EditProfileForm
        ...
        {friendRequests && props.user.username == user.username && (
          <FriendRequests
