  ...
  const readableData = () => {
    ...
    if(exercise != "Run" && exercise != "Bike"){
      for(const dataObj of userLifts){
        console.log(dataObj)
        list.push(<li>You lifted {Math.round(dataObj.weight)} pounds on {dataObj.date.slice(0,10)}</li>)
      }
    }else{
      for(const dataObj of userLifts){
        console.log(dataObj)
        list.push(<li>You did {dataObj.miles} miles on {dataObj.date.slice(0,10)}</li>);
      }
    }
    return list
  }
