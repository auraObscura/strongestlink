...
function Map(props) {
  ...
  const getMapMarkers = async () => {
    ...
    markerData.map(data => gymMaterialMarker.push({
      id: data.id, lat: data.latitude, lng: data.longitude, time: new Date(), name: data.name, description: data.description, type: data.type
    }))
  ...
  return (
    <div style={{ height: '90vh' }} >
      <Box position='absolute' left={0} top={20} h='90%' w='100%'>
        {/* Google Map Box */}
        <GoogleMap
          onClick={(event) => {
            setMarkers( [{
            ...
            setSelectedGym(null); setSelected(null);
          }}
          center={{ lat, lng }}
          zoom={15}
          ...
          {gymmarkers.map(marker => <Marker
            ...
            onClick={() => { setSelectedGym(marker); setSelected(null); setMarkers([]) }}
          />)}
          
          {selectedGym ?
            <InfoWindow className="infoWindow" onCloseClick={() => setSelectedGym(null)} position={{ lat: selectedGym.lat, lng: selectedGym.lng }}>
              ...
              <Button className='info-btn'>
                <NavLink className='gymLink' to={'location'}>Link to gym page</NavLink>
                </Button>
              <Button onClick={ () => { StrongestLinkApi.deletePin(selectedGym.id)} }  className='info-btn'>Delete</Button>
              </div>
      ...
      {/* ////////////////////// (Location of User ) ////////////////////////// */}
          ...
          {selected ?
            <InfoWindow 
            onCloseClick={() => { setSelected(null); setMarkers([])}} 
            position={{ lat: selected.lat, lng: selected.lng }}>
              ...
              lat={selected.lat} 
              lng={selected.lng}
              username={props.user} /></InfoWindow> : null}
          <Marker
            icon={{
